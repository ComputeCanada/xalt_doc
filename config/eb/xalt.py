...
class EB_XALT(ConfigureMake):
    ...
    def extra_options():
        extra_vars = {
            ...
            'file_prefix': [None, "Record files prefix", CUSTOM]
        }
    ...
    def configure_step(self):
        ...
        if self.cfg['file_prefix']:
            if self.cfg['transmission'] in ['file', 'file_separate_directory']:
                self.cfg.update('configopts', '--with-xaltFilePrefix=%s' % self.cfg['file_prefix'])
            else:
                error_msg = "A file prefix was specified, but the selected transmission style doesn't support it. "
                error_msg += "Only the 'file' and 'file_separate_directory' transmission styles support this option. "
                error_msg += "See https://xalt.readthedocs.io/en/latest/020_site_configuration.html#xalt-data-transmission for more information."
                raise EasyBuildError(error_msg)
